...
class UI_Window:
    def __init__(self, name='UI'):
        ...
        self.menu3.add_command(label='Convolution-2', command=self.funsP2('Convolution-2'))

        self.menu4.add_command(label='Image denoising', command=self.funsP3('Image denoising'))
        self.menu4.add_command(label='Bilateral filter', command=self.funsP3('Bilateral filter'))
        self.menu4.add_command(label='NLM', command=self.funsP3('NLM'))
        self.menu4.add_command(label='NLM improvement', command=self.funsP3('NLM improvement'))
    ...
    def funsP2(self, t):
        def fun():
            ...
            if t == 'Convolution-2':
                filt = self.filters.filterP3
                self.createNewWindowP2(filt)
            
        return fun

    def createNewWindowP3(self, filt):
        def fun():
            para = parameter.get(1.0, tk.END)
            para = para.split(',')
            if para[-1] == '\n':
    ...
    def funsP3(self, t):
        def fun():
            if t == 'Image denoising':
                filt = self.filters.order_stat_filt
                self.createNewWindowP3(filt)

            if t == 'Bilateral filter':
                filt = self.filters.bila_filt
                self.createNewWindowP3(filt)

            if t == 'NLM':
                filt = self.filters.non_local_mean_filt
                self.createNewWindowP3(filt)

            if t == 'NLM improvement':
                filt = self.filters.non_local_mean_filt_imp
                self.createNewWindowP3(filt)
